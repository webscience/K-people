...
public class ActivityDAO implements IActivityDAO {
    ...
    public final List<Activity> searchToDoActivities(
        ...
        // Controllo sui parametri formati
        ...
        try {
            newToDoActivities =
                    acUtil.syncToDoActivities(process, pattern, user);
        ...
        try {
            filteredActivities =
                    filterToDoActivities(process, pattern, user,
                            newToDoActivities);
    ...
    public final List<Activity> searchPendingActivities(
        ...
        try {
            usersToCheck =
                    calculateUserToCheckOnActiviti(process, pattern, user);
        ...
        if (usersToCheck != null) {
            for (int i = 0; i < usersToCheck.size(); i++) {
                ...
                List<Activity> actToDoForUser =
                        searchToDoActivities(pFilterToDo, user);

                for (int j = 0; j < actToDoForUser.size(); j++) {
                    Activity curAct = actToDoForUser.get(j);
                    if (curAct.getPattern().isWaitingActivity()
                            && curAct.getActivityRequestor().getIdUser() == user
                                    .getIdUser()) {
     ...
     */
    private List<Activity> filterToDoActivities(final Process pProcess,
            final Pattern pPattern, final User pUser,
            final List<Activity> pActivities) throws KPeopleSesameException {
        ...
        if (hpmPatternIdList.size() > 0) {
            for (int i = 0; i < activities.size(); i++) {
                ...
                if (hpmPatternIdList.contains(currentActivity.getPattern()
                        .getHpmPatternId())) {
    ...
    private List<String> calculateUserToCheckOnActiviti(final Process pProcess,
            final Pattern pPattern, final User pUser)
            throws KPeopleActivitiListTaskException, RepositoryException,
            MalformedQueryException, QueryEvaluationException, SQLException {
        ...
        if (process != null) {
            if (pattern != null) {
                userToCheckOnActivitiList.add(pattern.getPatternProvider()
                        .getHpmUserId());
            } else {
                ...
                if (listHpmPatternId.size() != 0) {
                    ...
                    List<Integer> listPatternId =
                            pDaoUtil.getPatternIdListFromHpmPatternIdList(listHpmPatternId);
                    patternIdStr =
                            pDaoUtil.createPatternIdStringList(listPatternId);
                    List<String> hpmProvidersId =
                            acDaoUtil
                                    .getHpmUsersIdByPatternRoleIdAndPatternIdList(
                                            Pattern.PATTERN_ROLE_PROVIDER,
                                            patternIdStr);

                    for (int i = 0; i < hpmProvidersId.size(); i++) {
                        if (!userToCheckOnActivitiList.contains(hpmProvidersId
                                .get(i)))
                            userToCheckOnActivitiList
                                    .add(hpmProvidersId.get(i));
                    }
                }
            }
        } else {
            // Non è definito nessun processo
            if (pattern != null) {
                // 1 solo pattern
                // Devo chiedere tutte le attività non fatte del provider
                userToCheckOnActivitiList.add(pattern.getPatternProvider()
                        .getHpmUserId());
            } else {
                List<String> ret =
                        acDaoUtil.getHpmProviderIdByRequestorId(user
                                .getIdUser());
     ...
     */
    public Activity executeActivity(final Activity pActivity, final User pUser)
            throws KPeopleActivitiPerformTaskOperationException {
