		...
		if (pagesIteratorEnd < pagesIteratorBegin) {
			...
			for (int i = pagesIteratorBegin; i <= pagesIteratorEnd; i++) {
				if (i == cur) {
					...
					sb.append(_getHREF(formName, curParam, i, jsCall, url, urlAnchor, request));
					sb.append("'>");
					sb.append(i);
					sb.append("</a>");
				}
	...
	<div class="search-pages">
		...
		<div class="page-links">
			<c:if test='<%= type.equals("more") || type.equals("regular") %>'>
				<c:choose>
					<c:when test="<%= cur != 1 %>">
						<a class="first no-decoration" href="<%= _getHREF(formName, curParam, 1, jsCall, url, urlAnchor, request) %>" target="<%= target %>">
					</c:when>
					<c:otherwise>
						<span class="first">
					</c:otherwise>
			...
			<c:choose>
				<c:when test="<%= cur != 1 %>">
					<a class="previous no-decoration" href="<%= _getHREF(formName, curParam, cur - 1, jsCall, url, urlAnchor, request) %>" target="<%= target %>">
				</c:when>
			...
			<c:choose>
				<c:when test="<%= cur != pages %>">
					<a class="next no-decoration" href="<%= _getHREF(formName, curParam, cur + 1, jsCall, url, urlAnchor, request) %>" target="<%= target %>">
				</c:when>
				<c:when test='<%= type.equals("more") || type.equals("regular") %>'>
					<span class="next">
				</c:when>
			...
			<c:if test='<%= type.equals("regular") %>'>
				<c:choose>
					<c:when test="<%= cur != pages %>">
						<a class="last no-decoration" href="<%= _getHREF(formName, curParam, pages, jsCall, url, urlAnchor, request) %>" target="<%= target %>">
					</c:when>
					<c:otherwise>
						<span class="last">
					</c:otherwise>
...
private String _getHREF(String formName, String curParam, int cur, String jsCall, String url, String urlAnchor, HttpServletRequest request) throws Exception {
    String href = url + curParam + "=" + cur + urlAnchor;
    
    String divId = (String)request.getAttribute("webscience-ui:search-iterator:divId");
    String formId = (String)request.getAttribute("webscience-ui:search-iterator:formId");
    
    jsCall = "KPeople.Common.load('" + divId + "','" + href + "', '" + formId + "');";
  
	return "javascript:" + jsCall;
}
%>