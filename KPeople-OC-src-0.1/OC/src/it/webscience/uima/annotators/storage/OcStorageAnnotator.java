...
public class OcStorageAnnotator extends JCasAnnotator_ImplBase {
    ...
    public final void process(final JCas cas)
        ...
        try {
            OcStorageResponse ocStoreResponse =
                storageAdapterAgent.storeEventData(cas);

            if (ocStoreResponse != null
                  && ocStoreResponse.getExitCode() == ok) {
                processCasProperties(cas, ocStoreResponse);
            } else {
                logger.fatal(
                    "exit code "
                     + ocStoreResponse.getExitCode());
                logger.fatal(
                    "error message: "
                    + ocStoreResponse.getErrorMessage());
     ...
     */
    private void processCasProperties(
            final JCas cas,
            final OcStorageResponse ocStorageResponse) {

        String xml = cas.getDocumentText();

        /**
         * mappa contenente il mapping tra id attachment ed url di alfresco
         * key: id attachment
         * value: url di alfresco
         */
        Hashtable<String, String> urlMap = ocStorageResponse.getUrlMap();

        String alfrescoUrl = Singleton.getInstance().
            getProperty("alfresco-public-url");

//      annotation relativa all'id legato all'evento
...
//      per ogni attachment, modifico la AttachmentAnnotation
