...
public class OcSolrAnnotator extends JCasAnnotator_ImplBase {
    ...
    private void processEvent(final JCas cas) {
        ...
        if (actionType != null) {
            eventProperties.setProperty(SolrIndexName.SOLR_ACTION_TYPE,
                    actionType);
        }
        // indice actionReference
        String actionReference = getActionReference(cas);
        if (actionReference != null) {
            eventProperties.setProperty(SolrIndexName.SOLR_ACTION_REF,
                    actionReference);
        }
        // indici per system
        String systemType = getSystemAnnotation(cas)[0];
        if (systemType != null) {
            eventProperties.setProperty(SolrIndexName.SOLR_SYSTEM_TYPE,
                    systemType);
        }
        ...
        if (creationDate != null) {
            eventProperties.setProperty(SolrIndexName.SOLR_EVENT_CREATION_DATE,
                    creationDate);
        }
    ...
    private String getOcStorageEventAnnotation(JCas cas) {
        ...
        Type annotationType = cas.getTypeSystem().getType(
                OcStorageEventAnnotation.class.getCanonicalName());
    ...
    private void getPropertyAnnotation(JCas cas) {
        ...
        Type annotationType = cas.getTypeSystem().getType(
                PropertyAnnotation.class.getCanonicalName());
        FSIterator<Annotation> iter = cas.getAnnotationIndex(annotationType)
                .iterator();
        ...
        while (iter.hasNext() && iter.isValid()) {
            ...
            if (propertyKey != null && propertyValue != null) {
                eventProperties.setProperty(SolrIndexName.SOLR_PROPERTY
    ...
    private void getDetailsAnnotation(JCas cas) {
        ...
        Type annotationType = cas.getTypeSystem().getType(
                UserAnnotation.class.getCanonicalName());
        FSIterator<Annotation> iter = cas.getAnnotationIndex(annotationType)
                .iterator();
        ...
        if (!UserReceiverBcc.isEmpty()) {
            eventProperties.setProperty(
                    SolrIndexName.SOLR_EVENT_DETAILS_RECEIVERBCC,
                    UserReceiverBcc);
        }

        if (!UserReceiverTo.isEmpty()) {
            eventProperties
                    .setProperty(SolrIndexName.SOLR_EVENT_DETAILS_RECEIVERTO,
                            UserReceiverTo);
        }

        if (!UserReceiverCc.isEmpty()) {
            eventProperties
                    .setProperty(SolrIndexName.SOLR_EVENT_DETAILS_RECEIVERCC,
                            UserReceiverCc);
        }
        if (logger.isDebugEnabled()) {
    ...
    private String getTitleAnnotation(JCas cas) {
        ...
        Type annotationType = cas.getTypeSystem().getType(
                TitleAnnotation.class.getCanonicalName());
        FSIterator<Annotation> iter = cas.getAnnotationIndex(annotationType)
                .iterator();
     ...
     */
    private void processAttachments(final JCas cas)
        throws UnsupportedEncodingException {

        if (logger.isDebugEnabled()) {
            logger.debug("Solr processAttachments - start");
        }
        AnnotationIndex<Annotation> attachmentAnnotation = cas
                .getAnnotationIndex(AttachmentAnnotation.type);
        ...
        while (itAttachment.hasNext()) {
            ...
            AttachmentAnnotation current =
                (AttachmentAnnotation) itAttachment.next();
...
//          viene eseguito l'encoding dell'url
            ...
            String encodedUrl =
                current.getUrlAttachment().substring(0, index + 1)
                + URLEncoder.encode(
                        current.getUrlAttachment().substring(index + 1),
                    "UTF-8");
            ...
            while (it.hasNext()) {
                ...
                if (idAttachment.equals(attachmentId)) {
                    ...
                    OcSolrProperties attachmentProperties =
                        new OcSolrProperties();
                    attachmentProperties.setProperty(SolrIndexName.SOLR_ID,
                            attachmentHashcode);
                    attachmentProperties.setProperty(
                            SolrIndexName.SOLR_ATTACHMENT_NAME, attachmentName);
                    attachmentProperties.setProperty(
                            SolrIndexName.SOLR_ATTACHMENT_TYPE, attachmentType);
                    attachmentProperties.setProperty(
                            SolrIndexName.SOLR_ATTACHMENT_URL, encodedUrl);
    ...
    private void getBodyAnnotation(final JCas cas) {
        ...
        Type annotationType = cas.getTypeSystem().getType(
                BodyAnnotation.class.getCanonicalName());
        FSIterator<Annotation> iter = cas.getAnnotationIndex(annotationType)
                .iterator();
        ...
        if (iter.isValid()) {
            ...
            eventProperties.setProperty(SolrIndexName.SOLR_EVENT_BODY,
                    ann.getValue());
            eventProperties.setProperty(SolrIndexName.SOLR_EVENT_BODY_TYPE,
                    ann.getBodyType());
        }
    ...
    private HashMap<String, Object> getUserAuthorAnnotation(final JCas cas) {
        ...
        Type annotationType = cas.getTypeSystem().getType(
                UserAuthorAnnotation.class.getCanonicalName());
        FSIterator<Annotation> iter = cas.getAnnotationIndex(annotationType)
                .iterator();
