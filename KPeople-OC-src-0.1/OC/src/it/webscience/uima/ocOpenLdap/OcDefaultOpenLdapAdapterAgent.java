...
public class OcDefaultOpenLdapAdapterAgent extends OcLdapAdapterAgent {
    ...
    public final List<OcLdapUser> search(final OcLdapUser ocLdapUser)
        ...
        logger.debug("Parametro in ingresso email: "
                + ocLdapUser.getEmail());
        logger.debug("Parametro in ingresso username: "
                + ocLdapUser.getUsername());
        logger.debug("Parametro in ingresso name: "
                + ocLdapUser.getName());

//      interroga cache/server in base all'email
        if (ocLdapUser.getEmail() != null) {
            searchEmail(ocLdapUser.getEmail());
        }

//      interroga cache/server in base all'username
        if (ocLdapUser.getUsername() != null) {
            searchUsername(ocLdapUser.getUsername());
        }

//      interroga cache/server in base al name
        if (ocLdapUser.getName() != null) {
            searchName(ocLdapUser.getName());
        }
...
//      1. ricerca senza carattery jolly
        double punteggioUsername = Double.parseDouble(
                singleton.getProperty("openldap.punteggi.username"));
...
//      2. ricerca con carattery jolly
        double punteggioEmailJolly = Double.parseDouble(
                singleton.getProperty(
                        "openldap.punteggi.username.jolly"));
